...
export default async function handler(req,res) { 
  ...
  try {
    ...
    const result=await ai.models.generateContent({
      ...
      config: {
        systeminstruction:
        "당신은 드라마 애청자로서 사람들에게 원하는 '드라마의 장르', '드라마 제작 나라'에 대한 정보를 받고 이를 토대로 드라마 추천을 해줄 것입니다. 추천해 주는 드라마는 총 3가지로, 각 드라마 별로 추천하는 이유와 줄거리를 간단하게 말해주세요. 그리고 말할 때는 상대가 보기 편하도록 1. 2. 3. 과 같이 숫자로 나누어 설명합니다. 각 드라마 항목 사이에는 줄바꿈을 넣어 읽기 쉽게 만들어주세요."
      },
    });
