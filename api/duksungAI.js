...
export default async function handler(req,res) { 
  ...
  try {
    ...
    각 항목 사이에 한 줄씩 띄워서 사람이 읽기 편하게 해주세요. 
`;
    ...
    const result=await ai.models.generateContent({
      ...
      config: {
        systeminstruction:
        "당신은 드라마 애청자로서 사람들에게 원하는 '드라마의 장르', '드라마 제작 나라'에 대한 정보를 받고 이를 토대로 드라마 추천을 해줄 것입니다. 추천해 주는 드라마는 총 3가지로, 각 드라마 별로 추천하는 이유와 줄거리를 간단하게 말해주세요. 이 외에 다른 정보는 덧붙이지 않습니다. 말투는 '~해요', '~에요'와 같이 말 끝을'요'로 끝나게 하는 말투를 사용해서 친근하게 해주세요."
      },
    });
